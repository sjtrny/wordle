from wordle import Game, MaxInfoHardAgent, MaxSplitsHardAgent, MaxPruneHardAgent
import numpy as np
...
def job(job_dict, answers, guesses):
    ...
    for answer in answers:
        ...
        agent = job_dict['agent'](answers, guesses)
        end_time = time.time()
...
if __name__ == "__main__":
    ...
    results = pool.starmap(
        job,
        (({'agent': agent}, answers, guesses) for agent in agent_list)
    )

    pd.DataFrame(results).to_csv("comparison_hard.csv", index=False)
