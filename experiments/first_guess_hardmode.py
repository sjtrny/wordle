...
from wordle import get_numeric_representations, get_bin_table_inline, bin_table_to_counts
...
if __name__ == "__main__":
    ...
    results.to_csv("first_guess_hardmode_results.csv", index=False)

    # Summarise results

    # results.loc[(results["answer"] != results["final_guess"]), "n_guesses"] = np.NaN

    # Filter out words that lead to failure

    failed_words = results[(results["answer"] != results["final_guess"])]["first_guess"].unique()
    # print(results[(results["answer"] != results["final_guess"])])
    ...
    summary.to_csv("first_guess_hardmode_results_summary.csv")
